...
class Admin {
	...
	public function ezd_post_states( $post_states, $post ) {
		...
		$docs_slug_page   = (int) ezd_get_opt( 'docs-slug' );
		$login_page       = ezd_is_premium() ? (int) ezd_get_opt( 'private_doc_login_page' ) : 0;
		$frontend_login   = ezd_is_promax() ? (int) ezd_get_opt( 'docs_frontend_login_page' ) : 0;
		...
		if ( $login_page === $frontend_login && $post->ID === $login_page ) {
			...
			if ( $post->ID === $login_page && 'login' === $private_mode ) {
