...
class Create_Post {
    ...
    public function create_parent_doc() {
	    ...
	    if ( isset( $_GET['parent_title'] ) && isset( $_GET['Create_doc'] ) && $_GET['Create_doc'] == 'yes' && isset( $_GET['_wpnonce'] )
	         && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'parent_doc_nonce' )
	    ) {

		    $title      = ! empty( $_GET['parent_title'] ) ? htmlspecialchars( sanitize_text_field( wp_unslash( $_GET['parent_title'] ) ) ) : 0;
	...
	public function create_new_doc() {
		...
		if ( isset( $_GET['new_doc'] ) && isset( $_GET['Create_doc'] ) && $_GET['Create_doc'] == 'yes' && isset( $_GET['_wpnonce'] )
		     && wp_verify_nonce( sanitize_text_field( wp_unslash( $_GET['_wpnonce'] ) ), 'create_new_doc_nonce' )
		) {

			$doc_title      = ! empty( $_GET['new_doc'] ) ? htmlspecialchars( sanitize_text_field( wp_unslash( $_GET['new_doc'] ) ) ) : 0;
			$str            = [ 'ezd_ampersand', 'ezd_hash', 'ezd_plus' ];
			$rplc           = [ '&', '#', '+' ];
	...
	public function create_section_doc() {
		...
		if ( isset($_GET['is_section']) && isset($_GET['Create_Section']) && $_GET['Create_Section'] == 'yes' && isset($_GET['_wpnonce']) && isset($_GET['parentID']) && wp_verify_nonce(wp_unslash($_GET['_wpnonce']), wp_unslash($_GET['parentID'])) ) {

			$parentID      			= ! empty ( $_GET['parentID'] ) ? absint( wp_unslash($_GET['parentID']) ) : 0;
			$section_title 			= ! empty ( $_GET['is_section'] ) ? htmlspecialchars( wp_unslash($_GET['is_section']) ) : '';
	...
	public function create_child_doc() {
		...
		if ( isset($_GET['child']) && isset($_GET['Create_Child']) && $_GET['Create_Child'] == 'yes' && isset($_GET['_wpnonce']) && isset($_GET['childID']) && wp_verify_nonce(wp_unslash($_GET['_wpnonce']), wp_unslash($_GET['childID'])) ) {

			$child_id    		= ! empty ( $_GET['childID'] ) ? absint( wp_unslash($_GET['childID']) ) : 0;
			$child_title 		= ! empty ( $_GET['child'] ) ? htmlspecialchars( wp_unslash($_GET['child']) ) : '';
