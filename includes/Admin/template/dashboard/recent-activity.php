<div class="ezd-card">
	<div class="ezd-card-header">
		<h2 class="ezd-card-title">
			...
			<?php esc_html_e( 'Recent Activity', 'eazydocs' ); ?>
		</h2>
	</div>
	<?php
	/**
	...
	$limit = 8; // number of activity items to show.
		...
		if ( empty( $activities ) ) :
			...
			<li class="ezd-activity-item ezd-activity-empty">
				<div class="ezd-empty-state">
					...
					<span><?php esc_html_e( 'No recent activity found.', 'eazydocs' ); ?></span>
				</div>
			</li>
			<?php
		else :
				...
				if ( 'comment' === $act['type'] ) :
					...
					switch ( $act['status'] ) {
						case 'publish':
							...
							$bg   = 'ezd-icon-bg-blue';
							$text = __( 'New article', 'eazydocs' );
							break;
						case 'draft':
							$icon = 'dashicons-edit';
							$bg   = 'ezd-icon-bg-yellow';
							$text = __( 'Draft updated', 'eazydocs' );
							break;
						case 'trash':
							$icon = 'dashicons-trash';
							$bg   = 'ezd-icon-bg-red';
							$text = __( 'Article deleted', 'eazydocs' );
							break;
						default:
							$icon = 'dashicons-admin-site-alt3';
							$bg   = 'ezd-icon-bg-gray';
							$text = __( 'Activity', 'eazydocs' );
							break;
					}
					?>
					<li class="ezd-activity-item">
