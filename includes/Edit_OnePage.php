...
class Edit_OnePage {
	...
	function edit_doc_one_page() {
		...
		if ( isset($_GET['edit_docs']) && isset($_GET['edit_onepage']) && $_GET['edit_onepage'] == 'yes' && isset($_GET['_wpnonce']) && wp_verify_nonce($_GET['_wpnonce'], $_GET['doc_id']) ) {
			...
			$page_id      		= sanitize_text_field( wp_unslash( $_GET['doc_id'] ?? '' ) );
			$layout       		= sanitize_text_field( wp_unslash( $_GET['layout'] ?? '' ) );
			$content_type 		= sanitize_text_field( wp_unslash( $_GET['content_type'] ?? '' ) );
			$content_type_right = sanitize_text_field( wp_unslash( $_GET['shortcode_right'] ?? '' ) );
			...
			if ( $content_type_right == 'widget_data_right' ) {
				...
				$page_content_rights     = esc_textarea( wp_unslash( $_GET['shortcode_content_right'] ?? '' ) );
			...
			if ( $content_type == 'widget_data' ) {
				$page_content 	= sanitize_text_field( wp_unslash( $_GET['left_side_sidebar'] ?? '' ) );
			} else {
				$page_contents 	= esc_textarea( wp_unslash( $_GET['edit_content'] ?? '' ) );
