...
class Frontend {
	...
	public function recently_viewed_docs( $title, $visibility, $visible_item, $see_more ) {
		$ft_cookie_posts = [];
		if ( isset( $_COOKIE['eazydocs_recent_posts'] ) ) {
			$cookie_val = sanitize_text_field( wp_unslash( $_COOKIE['eazydocs_recent_posts'] ) );
			$decoded    = json_decode( $cookie_val, true );
			if ( is_array( $decoded ) ) {
				$ft_cookie_posts = $decoded;
			}
		}

		$ft_cookie_posts = array_diff( $ft_cookie_posts, [ get_the_ID() ] );

		if ( ! empty( $ft_cookie_posts ) ) :
                ...
                <div class="topic_list_item">
                    ...
                    <ul class="navbar-nav recent-doc-list">
						...
						foreach ( $ft_cookie_posts as $postId ) :
							...
							if ( isset( $ft_post ) && in_array( $ft_post->post_type, [ 'docs' ], true ) ) {
								$count ++;
								?>
                                <li>
